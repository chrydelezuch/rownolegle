...
{
     // localId is calculated in right way, it is coalescing and there is no a problem with bank conflicts
    // subsequent threads are allocated to subsequent memory cells
